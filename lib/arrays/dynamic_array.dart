class DynamicArray<E extends Comparable> {
  List<E> _array;
  int length;
  int capacity;
  E defaultValue;

  DynamicArray({required this.capacity, required this.defaultValue})
      : length = 0,
  ...
  void doubleSize() {
    capacity *= 2;
    final newArray = List<E>.filled(capacity, defaultValue);
  ...
  void delete(E value) {
    ...
    if (length > 1 && length <= capacity / 4) {
      shrink();
    }
  }

  void shrink() {
    capacity = (capacity / 2).floor();
    final newArray = List<E>.filled(capacity, defaultValue);
    for (var i = 0; i < length; i++) {
      newArray[i] = _array[i];
    }
...
void main() {
  final array = DynamicArray<int>(capacity: 6, defaultValue: 0);
  array.insert(40);
  array.insert(2);
  array.insert(3);
  array.insert(4);
